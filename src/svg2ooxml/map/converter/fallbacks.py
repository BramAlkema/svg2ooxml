...
from .emf_adapter import EMFPathAdapter, PathStyle
    ...
    *,
    ...
    if style.stroke and style.stroke.paint and isinstance(style.stroke.paint, SolidPaint):
        stroke_color = hex_to_rgba(
            ...
            style.stroke.paint.opacity * shape_opacity,
        )
    ...
    if stroke_color is not None:
        line_points = scaled if scaled[0] == scaled[-1] else scaled + [scaled[0]]
        draw.line(line_points, fill=stroke_color, width=stroke_width, joint="curve")

    final_image = image.resize((width_px, height_px), PILImage.LANCZOS)
    buffer = BytesIO()
...
__all__ = ["render_emf_fallback", "render_bitmap_fallback"]
