...
This module centralises the geometry-heavy pieces that were previously embedded
inside ``drawingml.generator`` so the more advanced path handling described in
ADR-geometry-ir can be shared by multiple callers.
"""
...
def _evaluate_cubic(a: float, b: float, c: float, d: float, t: float) -> float:
