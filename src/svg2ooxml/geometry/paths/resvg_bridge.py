...
try:  # Optional dependency; import lazily to avoid circular init issues.
    from svg2ooxml.geometry.transforms.matrix import Matrix2D
except ImportError:  # pragma: no cover - only happens in constrained runtimes.
    Matrix2D = None  # type: ignore[assignment]

_TESSELLATOR = ResvgTessellator()


MatrixLike = ResvgMatrix | Sequence[float] | Sequence[Sequence[float]]
...
def normalize_path_to_segments(
    path_data: str | None,
    *,
    transform: MatrixLike | Matrix2D | None = None,
...
def _coerce_matrix(transform: MatrixLike | Matrix2D | None) -> ResvgMatrix:
    if transform is None:
        return ResvgMatrix.identity()
    if isinstance(transform, ResvgMatrix):
        return transform
    if Matrix2D is not None and isinstance(transform, Matrix2D):
...
def _convert_tessellation(result: TessellationResult) -> TessellationOutput:
    ...
    stroke_outline = (
        [[Point(x, y) for (x, y) in contour] for contour in result.stroke_outline]
        if result.stroke_outline
        else None
    )
