"""ViewBox resolution utilities mirroring the svg2pptx UVBTC layer."""
...
from svg2ooxml.transforms import Matrix
from svg2ooxml.units import UnitConverter
...
class ViewportEngine:
    """Resolve SVG viewBox + preserveAspectRatio semantics for the mapping stack."""
    ...
    @staticmethod
    def to_matrix(result: ViewBoxResult) -> Matrix:
...
def compute_viewbox(
    ...
    """Functional façade mirroring the previous svg2pptx helper."""
...
def viewbox_matrix_from_element(
    ...
    *,
    default_width: float = 800.0,
    default_height: float = 600.0,
) -> tuple[Matrix, ViewBoxResult]:
    """Convenience helper that returns the viewport matrix for ``svg_root``."""
