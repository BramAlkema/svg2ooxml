...
from svg2office.common.tempfiles import temporary_directory
from svg2office.drawingml.assets import FontAsset, NavigationAsset
from svg2office.drawingml.writer import DEFAULT_SLIDE_SIZE
from svg2office.io.pptx_assembly import (
...
from svg2office.ir.text import EmbeddedFontPlan
...
class _PackageWriterBase:
    ...
    def _required_xml_parts(self) -> dict[str, str]:
        return {
            "presProps.xml": self._build_pres_props_xml(),
            "viewProps.xml": _VIEW_PROPS_XML,
            "tableStyles.xml": _TABLE_STYLES_XML,
        }

    def _build_pres_props_xml(self) -> str:
        attrs = {
        ...
        if self._office_profile == OFFICE_PROFILE_COMPAT:
            attrs["xmlns:mc"] = NS_MC
            attrs["xmlns:p14"] = NS_P14
            attrs["mc:Ignorable"] = "p14"
            return (
                '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n'
                "<p:presentationPr "
                f'xmlns:a="{attrs["xmlns:a"]}" '
                f'xmlns:r="{attrs["xmlns:r"]}" '
                f'xmlns:p="{attrs["xmlns:p"]}" '
                f'xmlns:mc="{attrs["xmlns:mc"]}" '
                f'xmlns:p14="{attrs["xmlns:p14"]}" '
                'mc:Ignorable="p14">\n'
                "    <p:extLst>\n"
                f'        <p:ext uri="{EXT_URI_DISCARD_IMAGE_EDIT_DATA}">\n'
                f'            <p14:discardImageEditData val="{DISCARD_IMAGE_EDIT_DATA_VALUE}"/>\n'
                "        </p:ext>\n"
                f'        <p:ext uri="{EXT_URI_DEFAULT_IMAGE_DPI}">\n'
                f'            <p14:defaultImageDpi val="{DEFAULT_IMAGE_DPI_VALUE}"/>\n'
                "        </p:ext>\n"
                "    </p:extLst>\n"
                "</p:presentationPr>"
            )
    ...
    def _write_required_presentation_parts(self, package_root: Path) -> None:
        ...
        for name, content in self._required_xml_parts().items():
    ...
    def _ensure_theme_extension(self, package_root: Path) -> None:
        ...
        theme_family.set("name", "svg2office")
...
class StreamingPackageWriter(_PackageWriterBase):
    ...
    def begin(self) -> None:
        ...
        self._temp_dir_ctx = temporary_directory(prefix="svg2office_pptx_")
...
class PackageWriter(_PackageWriterBase):
    ...
    def write_package(
        ...
        *,
        ...
        try:
            with temporary_directory(prefix="svg2office_pptx_") as temp_path:
