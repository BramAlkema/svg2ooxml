...
from svg2office.io.emf import EMFRelationshipManager
from svg2office.ir.scene import Group, Image, SceneGraph
from svg2office.ir.text import TextFrame
...
if TYPE_CHECKING:  # pragma: no cover - type checking only
    ...
    from svg2office.services.image_service import ImageService
...
def _apply_mask_alpha(element, alpha: float):
    ...
    """
    from svg2office.ir.paint import (
        GradientStop,
        LinearGradientPaint,
        RadialGradientPaint,
        SolidPaint,
...
class DrawingMLWriter:
    ...
    def __init__(
        ...
        *,
        ...
        office_profile: str | None = None,
    ) -> None:
        self._template_dir = template_dir or _assets_root()
        self._image_service = image_service
        self._office_profile = normalize_office_profile(office_profile)
        ...
        self._next_media_index = 2
        self._next_navigation_index = 1
        self._picture_blip_extensions_by_rel: dict[str, dict[str, object]] = {}
    ...
    def _slide_root_profile_tokens(self) -> tuple[str, str]:
        """Return namespace and ignorable tokens for the slide root."""
        if self._office_profile != OFFICE_PROFILE_COMPAT:
            return "", ""
        xmlns = (
            f' xmlns:mc="{NS_MC}"'
            f' xmlns:a14="{NS_A14}"'
        ...
        ignorable = ' mc:Ignorable="a14 asvg"'
        return xmlns, ignorable
    ...
    def render_scene(
        ...
        *,
        ...
        self._next_media_index = 2
        self._next_navigation_index = 1
        self._picture_blip_extensions_by_rel.clear()
        ...
        self._shape_renderer = DrawingMLShapeRenderer(
            ...
            lookup_blip_extensions=self._lookup_blip_extensions,
            trace_writer=self._trace_writer,
        ...
        try:
            ...
            root_xmlns, root_ignorable = self._slide_root_profile_tokens()
            slide_xml = slide_xml.replace("{OFFICE_PROFILE_XMLNS}", root_xmlns)
            slide_xml = slide_xml.replace("{OFFICE_PROFILE_IGNORABLE}", root_ignorable)
            shapes_xml = "\n            ".join(fragments)
    ...
    def _register_media(self, image: Image) -> str:
        ...
        data_bytes = self._resolve_image_data(image)
        if data_bytes is None:
            logger.warning("Image data missing for %s; skipping media registration", image.href or "unknown")
            return ""
        if ext == "svg" and self._office_profile == OFFICE_PROFILE_COMPAT:
            fallback_r_id, fallback_filename = self._allocate_media_reference("png")
            source_r_id, source_filename = self._allocate_media_reference("svg")
            fallback_bytes = self._svg_fallback_png(data_bytes)
            self._assets.add_media(
                relationship_id=fallback_r_id,
            ...
            self._assets.add_media(
                relationship_id=source_r_id,
                filename=source_filename,
                data=data_bytes,
                content_type="image/svg+xml",
                source="image",
            )
            self._picture_blip_extensions_by_rel[fallback_r_id] = {
                "svg_relationship_id": source_r_id,
                "svg_extension_uri": EXT_URI_SVG_BLIP,
                "use_local_dpi": USE_LOCAL_DPI_VALUE,
            }
            self._trace_writer(
                "media_registered",
                stage="media",
            ...
            return fallback_r_id

        r_id, filename = self._allocate_media_reference(ext)
    ...
    def _allocate_media_reference(self, extension: str) -> tuple[str, str]:
        rid = f"rId{self._next_media_index}"
        filename = f"image{self._next_media_index}.{extension}"
        self._next_media_index += 1
        return rid, filename

    def _resolve_image_data(self, image: Image) -> bytes | None:
        data = image.data
    ...
    @staticmethod
    def _svg_fallback_png(svg_data: bytes) -> bytes:
        try:
            from resvg_py import svg_to_bytes

            svg_text = svg_data.decode("utf-8")
            rendered = svg_to_bytes(svg_string=svg_text)
            if isinstance(rendered, (bytes, bytearray)) and rendered:
                return bytes(rendered)
        except Exception:
            pass
        return DrawingMLWriter._minimal_png_1x1()
    ...
    def _lookup_blip_extensions(self, relationship_id: str) -> dict[str, object] | None:
        return self._picture_blip_extensions_by_rel.get(relationship_id)
