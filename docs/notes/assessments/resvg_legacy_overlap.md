...
The current codebase still has parser-era adapters alongside the resvg/usvg path.
`render.pipeline` consumes `core.resvg.usvg_tree.Tree` directly; the render
package must not contain a second tree or paint normalizer.

| Concern | Legacy path (svg2pptx era) | Resvg/usvg path | Bridging today | Duplication impact |
| --- | --- | --- | --- | --- |
| **Parsing & tree normalisation** | Parser-era DOM/style extraction still feeds fallback metadata | `core/resvg/parser/*`, `core/resvg/usvg_tree.py` build a usvg-compatible tree with full inheritance/unit resolution | `IRConverter._build_resvg_lookup` computes a map from DOM nodes → resvg nodes | No render-package tree normalizer remains; remaining risk is fallback metadata diverging from resvg |
...
1. **CSS/presentation cascade still has fallback adapters.** The old render normalizer is gone, but parser-era style extraction remains as fallback metadata. Subtle differences (e.g., shorthand expansion, white-space rules) can still affect downstream systems that consume fallback style results.
...
3. **Keep the render package resvg-only.** Do not reintroduce a parallel tree or paint parser under `render/`. New render helpers should consume `core.resvg.usvg_tree.Tree` or typed resvg descriptors.
