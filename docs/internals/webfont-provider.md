...
**Scope**: Current `@font-face` provider registration, resolution behavior, and integration points inside the conversion pipeline.
...
This document describes the current provider-side wiring and matching behavior. It does not own the staged rollout for font loading, decompression, or embedding policy. Those follow-on decisions belong in:
...
## Current Limitations and Follow-On Work
...
### Follow-On Work (Tracked in Spec/Tasks)

**Font Loading / Decompression**
...
Keep the sequencing and acceptance criteria for those items in the spec/task docs, not in this implementation snapshot.
