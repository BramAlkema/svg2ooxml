...
*   **`begin` Coverage is Partial (but no longer time-only):** Event-based begin triggers are supported for `click`, `click+offset` (for example `begin="click+0.5s"` and `begin="click + 0.5s"`), and element sync (`id.begin` / `id.end` with optional offsets). `begin="indefinite"` can remap to a bookmark click trigger, and deterministic integer `element.repeat(n)` cases can expand to time offsets; `accessKey`, `wallclock`, and generic DOM events remain explicit unsupported runtime triggers.
