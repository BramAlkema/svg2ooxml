...
bugs; sibling `openxml-audit/docs/pptx_oracle/ssot.md` for the oracle
methodology.
...
| `<animate>` text color | `<p:animClr>` on `style.color` | `emph/text_color` | Done | Visually verified. Simple SMIL text-color paths are handler-gated. |
...
| Text color change | `text_color` | `emph/text_color` or compound | Done (gated) | Restricted to actual text targets; do not reuse the text preset for shape-level `color`. |
| Stroke color change | `stroke_color` | `emph/stroke_color` or compound | Done (verified) |
| Color pulse (autoReverse) | — | `emph/color_pulse` | Done (gated) | Text-only `orig -> pulse -> orig` matcher. Uses paragraph build mode. |
...
| Blink (visibility toggle) | `blink` | `emph/blink` or compound | Done (gated) | Visibility compiler routes bounded hide/show pulses that return to visible state. |
| Stacked multi-behavior | N behaviors composed | `emph/compound` | Partial | Compound slot verified. Writer still needs same-target timing collation. |
...
| `begin="element.repeat(n)"` | Deterministic integer repeat events expand to time offsets | Partial | Integer `n` works when the base timing is resolvable; fractional or ambiguous repeat events remain policy-skipped. |
...
| `repeatCount="n"` | `<p:cTn repeatCount="n*1000">`, fractional counts lowered to `repeatDur` cap | Done | Integer counts map directly; fractional counts preserve active duration via `repeatDur`. |
...
| `min` / `max` | Duration clamp before emission | Partial | Writer clamps authored duration pre-emission; no dedicated PPT timing attr exists. |
...
### 8.9 Remaining Closure Ledger

Decision labels are normative: `wire`, `gate`, `mimic`, `dead`, `fallback`.
Definitions live in
`docs/specs/svg-animation-native-mapping-spec.md` section 13.

| Remaining SMIL case | Candidate native/oracle path | Required gate or limitation | Decision | Next slice |
|---|---|---|---|---|
| Multiple simultaneous emphasis behaviors on one target | `emph/compound` | Same target shape, same begin/duration/repeat/fill/restart semantics, compatible text build mode, no conflicting target attrs | `gate` | Add writer collation pass that groups compatible sibling animations before handler emission. |
| `animateTransform type="matrix"` with clean decomposition | `emph/compound` with `motion` + `scale` + `rotate` fragments | Only when the matrix decomposes without residual skew/shear ambiguity or unsupported reflection semantics | `gate` | Wire clean matrix decompositions; send the rest to skew/morph/flipbook policy. |
| Stroke-dashoffset used as line reveal | `entr/filter_effect` with directional `wipe(...)` | This is only a reveal mimic, not exact dash semantics. Direction must be inferred/documented; current baseline is `wipe(right)` | `mimic` | Replace the hard-coded baseline with a documented direction heuristic and proof deck. |
| Animated `stroke-dasharray` / dash-phase effects beyond simple reveal | None exact; limited `wipe(...)` mimic for reveal-only cases | Native PPT has no editable dash-phase runtime model. Only reveal-like cases may borrow wipe; patterned dash animation stays non-native | `fallback` | Keep reveal-only mimic narrow; otherwise route to flipbook/morph/static resolution. |
| Isolated font-size animation on text | Raw `style.fontSize` anim or preset-28-style compound | Standalone `style.fontSize` is a confirmed dead path. Do not emit it unless a verified text-size compound slot exists | `dead` | Never emit isolated font-size timing XML; add a dedicated oracle slot before claiming native support. |
| Render-tree `display` animation compiled to descendant visibility | Compiled `style.visibility` `<p:set>` intervals | Only after leaf targets, inheritance, and static render suppression are resolved up front | `wire` | Finish writer coverage and visual tests for compiled descendant visibility plans. |
| Motion path `rotate="auto"` / `auto-reverse` | `<p:animMotion>` + sampled `<p:animRot>` | Angle is approximate unless tangent sampling error is bounded; cusps and zero-length segments need explicit skip/degrade behavior | `mimic` | Define the tangent-error budget and add proof decks for path classes that stay within it. |
| `begin="element.begin"` / `begin="element.end"` | Native event-linked start conditions | Target IDs must resolve to emitted timing/shape IDs, and playback must be oracle/visually confirmed before claiming exact support | `gate` | Build a timing proof deck for event-linked begin conditions. |
| `end`, `repeatDur`, and `restart` timing overrides | Native `<p:endCondLst>`, `repeatDur`, `restart` | XML is wired, and `openxml_audit.pptx.timing_oracle_deck` / `openxml-audit-pptx-timing-oracle` now owns the slideshow probe deck; runtime behavior still needs observation before we call it exact | `gate` | Run the timing oracle deck in PowerPoint, record observed behavior, then upgrade statuses only where playback matches authored SMIL. |
| `min` / `max` duration constraints | Pre-emit duration clamp in writer | This is a writer-side normalization, not a native PPT timing primitive; semantics must stay documented as a clamp, not a runtime feature | `wire` | Add tests/docs around clamped duration behavior and keep it out of exact-native timing claims. |
| `calcMode="spline"`, `keySplines`, `keyPoints` | Sampled/expanded native timing | Exact cubic timing is not available in PPT; all such paths need an explicit error threshold and mimic labeling | `mimic` | Finish the proof deck and publish acceptable error bounds per primitive. |
