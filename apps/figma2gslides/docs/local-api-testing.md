...
Run the extracted FastAPI service on your laptop against the real Firestore/Drive stack so
...
python apps/figma2gslides/tools/local_api.py setup

# Or the legacy bash script
bash apps/figma2gslides/tools/local_api.sh setup
```
...
```bash
# Python CLI (recommended)
python apps/figma2gslides/tools/local_api.py run

# Or the legacy bash script
bash apps/figma2gslides/tools/local_api.sh run
```
...
```bash
SVG2OOXML_LOCAL_PORT=8081 python apps/figma2gslides/tools/local_api.py run
```
...
python tests/hosted_service/batch_export_w3c.py --limit 1
```
...
def test_export_endpoint(api_base_url):
    ...
    assert response.json()["service"] == "figma2gslides-export"
```
...
```bash
SVG2OOXML_TEST_PORT=9000 pytest tests/integration/export/
```
...
```python
from figma2gslides.api.testing import LocalAPIServer, LocalAPIConfig
