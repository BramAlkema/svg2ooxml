...
if TYPE_CHECKING:  # pragma: no cover - type-checking only
    from figma2gslides.auth import GoogleOAuthService
...
def google(status: bool, revoke: bool) -> None:
    ...
    try:
        from figma2gslides.auth import (  # noqa: WPS433 - optional dependency import
            GoogleOAuthService,
            OAuthError,
            get_cli_token_store,
            get_system_username,
        ...
        click.echo(
            "❌ Google integrations require the app auth dependencies.",
            err=True,
        )
    ...
    if status:
        ...
        if info is None:
            click.echo("❌ Not authenticated. Start the figma2gslides auth flow to connect.")
        else:
