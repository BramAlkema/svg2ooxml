...
def test_filter_raster_fallback_uses_resvg_renderer() -> None:
    ...
    descriptor = resolve_filter_element(_filter_element("<filter id='blur'><feGaussianBlur stdDeviation='2'/></filter>"))
    filter_service.register_filter("blur", descriptor)
    ...
    effects = filter_service.resolve_effects("blur", context=context)
